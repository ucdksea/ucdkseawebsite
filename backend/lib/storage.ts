...
function urlToS3Key(u: URL): string {
  return decodeURIComponent(u.pathname.replace(/^\/+/, ''));
}

/** URL → 로컬 파일 경로 (public 하위만 허용) */
function urlToLocalPath(u: URL): string | null {
...
/** 로컬 파일 삭제 */
async function deleteLocalByUrl(urlStr: string): Promise<DeleteResult> {
  try {
    const u = new URL(urlStr, 'http://dummy'); // 상대경로 허용
    const p = urlToLocalPath(u);
    if (!p) return { ok: false, error: 'Unsafe path' };
    await fs.unlink(p).catch((e: any) => {
      if (e?.code === 'ENOENT') return; // 이미 없으면 성공 간주
      throw e;
    });
    return { ok: true };
  } catch (e: any) {
...
async function deleteS3ByUrl(urlStr: string): Promise<DeleteResult> {
  ...
  try {
    ...
    const mod: any = await import(pkg as any).catch(() => null);
    if (!mod) return { ok: false, error: '@aws-sdk/client-s3 not installed' };
...
export async function deleteImageByUrl(urlStr?: string | null): Promise<DeleteResult> {
